...
# Проверка процесса
if pgrep -f "uvicorn core.api.main" > /dev/null; then
...
response=$(curl -s -w "\n%{http_code}" http://localhost:8000/api/health 2>&1)
...
echo ""
echo "📡 Проверка /api/yandex-metrika/counters..."
response=$(curl -s -w "\n%{http_code}" http://localhost:8000/api/yandex-metrika/counters 2>&1)
http_code=$(echo "$response" | tail -n1)
body=$(echo "$response" | head -n-1)

if [ "$http_code" = "200" ]; then
    echo "✅ Метрика endpoint работает (HTTP $http_code)"
    echo "   Найдено счетчиков: $(echo "$body" | grep -o '"id"' | wc -l | tr -d ' ')"
