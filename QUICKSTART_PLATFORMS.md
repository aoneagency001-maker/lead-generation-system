# 🚀 Quick Start: OLX & Satu Модули

**Статус**: ✅ Инфраструктура готова, требуется тестирование

---

## 📋 Что создано

### OLX Module (Port 8001)
- ✅ OAuth 2.0 авторизация
- ✅ 3 метода парсинга (lerdem, playwright, api)
- ✅ Playwright publisher
- ✅ Python SDK
- ✅ 10 таблиц в Supabase

### Satu Module (Port 8002)
- ✅ API Token авторизация
- ✅ 2 метода парсинга (playwright, api)
- ✅ API publisher
- ✅ Python SDK
- ✅ 10 таблиц в Supabase

---

## ⚡ Быстрый старт (3 шага)

### Шаг 1: База данных (5 минут)

```bash
# Откройте Supabase Dashboard → SQL Editor
# Выполните по очереди:

# 1. OLX схема
cat modules/platforms/olx/database/schema.sql | pbcopy
# Вставьте в SQL Editor и выполните

# 2. Satu схема
cat modules/platforms/satu/database/schema.sql | pbcopy
# Вставьте в SQL Editor и выполните

# Проверьте что таблицы созданы
```

### Шаг 2: Credentials (10 минут)

#### OLX (OAuth 2.0)
1. Зайти на https://developer.olx.kz
2. Sign In с вашим OLX аккаунтом
3. Создать приложение
4. Получить Client ID и Client Secret

#### Satu (API Token)
1. Зайти на https://satu.kz (аккаунт компании)
2. Настройки → Управление API-токенами
3. Создать токен с правами: Orders, Products, Groups, Clients
4. Скопировать токен

#### .env файл
```bash
# Создайте .env в корне проекта
cat > .env << 'EOF'
# OLX
OLX_CLIENT_ID=your_client_id
OLX_CLIENT_SECRET=your_client_secret
OLX_REDIRECT_URI=http://localhost:8001/auth/callback

# Satu
SATU_API_TOKEN=your_api_token
SATU_COMPANY_ID=your_company_id

# Supabase
SUPABASE_URL=https://xxx.supabase.co
SUPABASE_KEY=your_key

# Browser
BROWSER_HEADLESS=true
BROWSER_SLOW_MO=50

# Parser
PARSER_TIMEOUT=30
EOF
```

### Шаг 3: Запуск (2 минуты)

```bash
# Terminal 1 - OLX Module
cd modules/platforms/olx
pip install -r requirements.txt
python -m api.main

# Откроется на http://localhost:8001
# Swagger docs: http://localhost:8001/docs

# Terminal 2 - Satu Module
cd modules/platforms/satu
pip install -r requirements.txt
python -m api.main

# Откроется на http://localhost:8002
# Swagger docs: http://localhost:8002/docs
```

---

## 🧪 Тестирование через Swagger UI

### OLX (http://localhost:8001/docs)

#### 1. Авторизация
```http
POST /auth/oauth/login
{
  "email": "your@email.com",
  "password": "your_password"
}

Response:
{
  "id": "uuid",
  "access_token": "...",
  "refresh_token": "..."
}
```

#### 2. Парсинг
```http
POST /parser/search
{
  "search_query": "iphone 15",
  "city": "almaty",
  "parser_method": "lerdem",
  "max_pages": 1
}

Response:
{
  "success": true,
  "task_id": "uuid",
  "status": "pending"
}

# Проверить статус
GET /parser/tasks/{task_id}

# Получить результаты
GET /parser/results/{result_id}
```

#### 3. Публикация
```http
POST /ads?account_id={account_id}
{
  "title": "iPhone 15 Pro 256GB",
  "description": "Новый, запечатанный",
  "price": 450000,
  "category": "electronics",
  "city": "almaty",
  "images": []
}

Response:
{
  "success": true,
  "ad_url": "https://www.olx.kz/d/...",
  "external_id": "123456"
}
```

---

### Satu (http://localhost:8002/docs)

#### 1. Авторизация
```http
POST /auth/token
{
  "api_token": "your_token",
  "company_id": "your_company_id"
}

Response:
{
  "id": "uuid",
  "company_name": "..."
}
```

#### 2. Парсинг
```http
POST /parser/search
{
  "search_query": "сварочный аппарат",
  "parser_method": "playwright",
  "max_pages": 1
}

Response:
{
  "success": true,
  "task_id": "uuid"
}

# Проверить статус и получить результаты
GET /parser/tasks/{task_id}
GET /parser/results/{result_id}
```

#### 3. Публикация
```http
POST /products?account_id={account_id}
{
  "title": "Сварочный аппарат Ресанта САИ-190",
  "description": "Инверторный, 190А",
  "price": 85000,
  "quantity": 10,
  "category": "instruments",
  "images": []
}

Response:
{
  "success": true,
  "ad_url": "https://satu.kz/product/..."
}
```

---

## 🐍 Python SDK Examples

### OLX SDK

```python
import asyncio
from modules.platforms.olx.sdk import OLXClient

async def main():
    async with OLXClient("http://localhost:8001") as client:
        # Авторизация
        account = await client.auth.oauth_login(
            email="your@email.com",
            password="password"
        )
        print(f"Logged in: {account['email']}")
        
        # Парсинг
        print("\n🔍 Parsing OLX...")
        task = await client.parser.search("iphone 15", city="almaty")
        results = await client.parser.wait_for_results(task["task_id"])
        
        print(f"✅ Found: {results['items_count']} ads")
        for item in results["data"][:5]:  # Первые 5
            print(f"  - {item['title']}: {item['price']} тг")
        
        # Публикация
        print("\n📤 Publishing ad...")
        ad = await client.ads.create(
            title="iPhone 15 Pro 256GB",
            description="Новый, запечатанный",
            price=450000,
            category="electronics",
            city="almaty",
            account_id=account["id"]
        )
        print(f"✅ Published: {ad['ad_url']}")

if __name__ == "__main__":
    asyncio.run(main())
```

### Satu SDK

```python
import asyncio
from modules.platforms.satu.sdk import SatuClient

async def main():
    async with SatuClient("http://localhost:8002") as client:
        # Авторизация
        account = await client.auth.login(api_token="your_token")
        print(f"Logged in: {account['company_name']}")
        
        # Парсинг
        print("\n🔍 Parsing Satu...")
        task = await client.parser.search("сварочный аппарат")
        results = await client.parser.wait_for_results(task["task_id"])
        
        print(f"✅ Found: {results['items_count']} products")
        for item in results["data"][:5]:  # Первые 5
            print(f"  - {item['title']}: {item['price']} тг")
        
        # Публикация
        print("\n📤 Publishing product...")
        product = await client.products.create(
            title="Сварочный аппарат Ресанта САИ-190",
            description="Инверторный, 190А",
            price=85000,
            quantity=10,
            category="instruments",
            account_id=account["id"]
        )
        print(f"✅ Published: {product['ad_url']}")

if __name__ == "__main__":
    asyncio.run(main())
```

---

## 📁 Структура файлов

```
modules/platforms/
├── olx/
│   ├── api/
│   │   ├── main.py              ← FastAPI app (port 8001)
│   │   ├── routes/
│   │   │   ├── auth.py          ← OAuth endpoints
│   │   │   ├── parser.py        ← Парсинг
│   │   │   └── ads.py           ← Публикация
│   │   └── config.py
│   ├── services/
│   │   ├── auth_service.py      ← OAuth logic
│   │   ├── parser_service.py    ← lerdem adapter
│   │   ├── parser_playwright.py ← Playwright parser
│   │   └── publisher_service.py ← Публикация
│   ├── database/
│   │   ├── schema.sql           ← 10 таблиц
│   │   └── client.py
│   ├── models.py                ← Pydantic models
│   ├── sdk/
│   │   └── __init__.py          ← Python SDK
│   └── requirements.txt
│
└── satu/
    └── (аналогичная структура)
```

---

## 🔍 Debugging

### Logs
```bash
# Логи пишутся в консоль
# Уровень: INFO (можно изменить в config.py)

# Если ошибка парсинга - проверьте селекторы
# Если ошибка авторизации - проверьте credentials
```

### Common Issues

**Ошибка: "Account not found"**
```bash
# Сначала авторизуйтесь через /auth endpoints
# Затем используйте полученный account_id
```

**Ошибка: "Task status: failed"**
```bash
# Проверьте логи модуля
# Возможно изменилась структура HTML (обновите селекторы)
```

**Ошибка: "Browser timeout"**
```bash
# Увеличьте PARSER_TIMEOUT в .env
# Или отключите headless: BROWSER_HEADLESS=false
```

---

## 📚 Дополнительная документация

- **Полный отчет**: `/docs/PLATFORMS_COMPARISON_REPORT.md`
- **Архитектура**: `/docs/PLATFORM_MODULES_PROGRESS_REPORT.md`
- **Database schemas**: 
  - `/modules/platforms/olx/database/schema.sql`
  - `/modules/platforms/satu/database/schema.sql`

---

## 🎯 Next Steps

1. ✅ Применить SQL схемы в Supabase
2. ✅ Получить credentials (OLX OAuth, Satu Token)
3. ✅ Запустить модули
4. ✅ Протестировать через Swagger UI
5. ⏸️ Интегрировать в основную систему через SDK
6. ⏸️ Добавить в n8n workflows
7. ⏸️ Настроить мониторинг и alerts

---

**Статус**: ✅ **Готово к запуску**

Вопросы? Проблемы? Проверьте логи и `/docs/PLATFORMS_COMPARISON_REPORT.md`


