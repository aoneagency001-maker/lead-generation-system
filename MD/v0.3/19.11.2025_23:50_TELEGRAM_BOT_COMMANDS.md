# Telegram Bot - Команды и Тестирование

**Дата создания:** 19.11.2025 23:50

## 🎯 Что добавлено

Теперь твой Telegram бот **интерактивный** - можешь отправлять ему команды и получать ответы!

### ✅ Доступные команды:

- `/start` - Приветствие и список команд
- `/status` - Статус системы (uptime, memory, database)
- `/health` - Детальная проверка здоровья всех сервисов
- `/stats` - Статистика за последние 24 часа
- `/help` - Справка по всем командам

---

## 🚀 Как запустить бота

### Вариант 1: Через скрипт (рекомендуется)

```bash
cd /Users/vbut/lead-generation-system
./scripts/start_telegram_bot.sh
```

### Вариант 2: Напрямую через Python

```bash
cd /Users/vbut/lead-generation-system
python -m shared.telegram_bot
```

### Вариант 3: В фоне (для production)

```bash
# Используй screen или tmux
screen -S telegram_bot
python -m shared.telegram_bot
# Нажми Ctrl+A, затем D для отсоединения

# Или через nohup
nohup python -m shared.telegram_bot > telegram_bot.log 2>&1 &
```

---

## 📱 Использование команд

### 1. `/start` - Приветствие

**Отправь боту:**
```
/start
```

**Получишь:**
```
🤖 Lead Generation System Bot

Welcome! I'm your system monitoring bot.

Available commands:
/status - System status
/health - Health check
/stats - Statistics (24h)
/help - Help
```

---

### 2. `/status` - Статус системы

**Отправь боту:**
```
/status
```

**Получишь:**
```
📊 System Status

🟢 Status: Running
⏰ Uptime: N/A
💾 Memory: N/A
🗄️ Database: ✅ OK
🌍 Environment: Development
📦 Version: 0.1.0
```

---

### 3. `/health` - Health Check

**Отправь боту:**
```
/health
```

**Получишь:**
```
🏥 Health Check

Overall Status: ✅ Healthy

Services:
• Database: ✅ OK
• Telegram: ✅ Configured
• WhatsApp: ⚠️ Not checked
• Redis: ⚠️ Not checked

✅ All services operational
```

**Если есть проблемы:**
```
🏥 Health Check

Overall Status: ⚠️ Degraded

Services:
• Database: ❌ Error: Connection timeout
• Telegram: ✅ Configured
• WhatsApp: ⚠️ Not checked
• Redis: ⚠️ Not checked

⚠️ Issues detected:
• Database unreachable
```

---

### 4. `/stats` - Статистика

**Отправь боту:**
```
/stats
```

**Получишь:**
```
📈 Statistics (Last 24h)

👥 Leads created: 42
🚀 Active campaigns: 3
❌ Errors: 0

⏰ Last updated: 2025-11-19 23:50:12
```

---

### 5. `/help` - Справка

**Отправь боту:**
```
/help
```

**Получишь:**
```
📖 Available Commands

/start - Welcome message
/status - System status (uptime, memory, database)
/health - Detailed health check of all services
/stats - Statistics for last 24 hours
/help - This help message

Examples:
• Send /status to check if system is running
• Send /health to verify all services
• Send /stats to see recent activity

Note: This bot also sends automatic notifications about errors and system events.
```

---

## 🧪 Тестирование бота

### Полный тест всех функций

Запусти тестовый скрипт:

```bash
cd /Users/vbut/lead-generation-system
python scripts/test_telegram_bot.py
```

**Что тестируется:**

1. ✅ Success notification
2. ✅ Warning notification
3. ✅ Error notification
4. ✅ Critical notification
5. ✅ Command /start
6. ✅ Command /status
7. ✅ Command /health
8. ✅ Command /stats
9. ✅ Command /help
10. ✅ Error handling

**Результат:**
```
🧪 TELEGRAM BOT TESTING SUITE
============================================================

📋 Running: 1. Success Notification
✅ 1. Success Notification - PASSED

📋 Running: 2. Warning Notification
✅ 2. Warning Notification - PASSED

...

📊 TEST RESULTS
============================================================
✅ Passed: 10
❌ Failed: 0
📊 Total: 10
============================================================
```

**Проверь Telegram** - должно прийти:
- 4 уведомления (success, warning, error, critical)
- 5 ответов на команды

---

## 🔧 Ручное тестирование

### Тест 1: Уведомления

```python
# test_notifications.py
import asyncio
from shared.telegram_notifier import telegram_notifier

async def test():
    # Success
    await telegram_notifier.send_success("Test success!", module="Test")
    
    # Warning
    await telegram_notifier.send_warning("Test warning!", module="Test")
    
    # Error
    try:
        raise ValueError("Test error")
    except Exception as e:
        await telegram_notifier.send_error(e, module="Test")
    
    # Critical
    await telegram_notifier.send_critical(
        "Test critical!",
        module="Test",
        details={"test": "value"}
    )

asyncio.run(test())
```

**Запуск:**
```bash
python test_notifications.py
```

---

### Тест 2: Команды бота

**Важно:** Бот должен быть запущен!

1. Запусти бота в одном терминале:
```bash
python -m shared.telegram_bot
```

2. В Telegram отправь команды:
- `/start`
- `/status`
- `/health`
- `/stats`
- `/help`

3. Проверь что получаешь ответы

---

### Тест 3: Интеграция с системой

**Тест автоматических уведомлений:**

1. Запусти FastAPI сервер:
```bash
uvicorn core.api.main:app --reload
```

2. Вызови endpoint который упадет:
```bash
curl http://localhost:8000/api/test-error
```

3. Проверь Telegram - должно прийти уведомление об ошибке

---

## 🎨 Примеры использования в коде

### Пример 1: Уведомление при создании лида

```python
# modules/3-lead-qualification/service.py
from shared.telegram_notifier import notify_success

async def create_lead(phone: str, name: str):
    lead = await db.leads.create({
        "phone": phone,
        "name": name
    })
    
    # Уведомить в Telegram
    await notify_success(
        f"✅ New lead created!\n"
        f"Phone: {phone}\n"
        f"Name: {name}",
        module="LeadService"
    )
    
    return lead
```

---

### Пример 2: Мониторинг парсинга

```python
# modules/1-market-research/parser.py
from shared.telegram_notifier import telegram_notifier
import time

async def parse_olx(category: str):
    start = time.time()
    
    try:
        ads = await _parse(category)
        elapsed = time.time() - start
        
        # Отчет в Telegram
        await telegram_notifier.send_success(
            f"✅ OLX parsing completed!\n"
            f"Category: {category}\n"
            f"Ads found: {len(ads)}\n"
            f"Time: {elapsed:.1f}s",
            module="OLXParser"
        )
        
        return ads
    
    except Exception as e:
        await telegram_notifier.send_error(
            error=e,
            module="OLXParser",
            extra_info={"category": category}
        )
        raise
```

---

### Пример 3: Критическое событие

```python
# core/monitoring/disk_check.py
from shared.telegram_notifier import telegram_notifier

async def check_disk_space():
    usage = get_disk_usage_percent()
    
    if usage > 90:
        await telegram_notifier.send_critical(
            message="Disk space critical!",
            module="SystemMonitor",
            details={
                "disk_usage": f"{usage}%",
                "action": "Clean up or expand disk ASAP"
            }
        )
```

---

## 🔐 Безопасность

### Авторизация

Бот принимает команды **только от авторизованного пользователя** (твой CHAT_ID).

Если кто-то другой попытается использовать бота:
```
❌ Unauthorized. Only authorized users can use this bot.
```

### Как добавить еще пользователей

В `shared/telegram_bot.py` измени проверку:

```python
# Было:
if str(chat_id) != str(self.chat_id):
    # Отклонить

# Стало (множественные пользователи):
authorized_users = [
    os.getenv("TELEGRAM_CHAT_ID"),
    "987654321",  # Другой пользователь
    "123456789"   # Еще один
]

if str(chat_id) not in authorized_users:
    # Отклонить
```

Или через переменную окружения:
```bash
# .env
TELEGRAM_AUTHORIZED_USERS=123456789,987654321,111222333
```

---

## 🐛 Troubleshooting

### Бот не отвечает на команды

**Проверь:**
1. ✅ Бот запущен: `ps aux | grep telegram_bot`
2. ✅ TOKEN правильный в .env
3. ✅ CHAT_ID правильный в .env
4. ✅ Написал боту `/start` перед командами

**Логи:**
```bash
# Если запущен через nohup
tail -f telegram_bot.log

# Или запусти с выводом
python -m shared.telegram_bot
```

---

### Команды работают, но уведомления не приходят

**Проверь:**
1. ✅ `telegram_notifier` используется в коде
2. ✅ Ошибки действительно происходят
3. ✅ Логи показывают отправку: `grep Telegram logs/`

**Тест:**
```python
python -c "import asyncio; from shared.telegram_notifier import telegram_notifier; asyncio.run(telegram_notifier.send_success('Test'))"
```

---

### "Unauthorized" при отправке команд

**Причина:** CHAT_ID не совпадает

**Решение:**
1. Проверь свой CHAT_ID:
```bash
curl "https://api.telegram.org/bot<TOKEN>/getUpdates"
```

2. Обнови .env:
```bash
TELEGRAM_CHAT_ID=правильный_chat_id
```

3. Перезапусти бота

---

## 📊 Мониторинг бота

### Проверка что бот работает

```bash
# Проверить процесс
ps aux | grep telegram_bot

# Проверить логи
tail -f telegram_bot.log

# Отправить тестовую команду
curl -X POST "https://api.telegram.org/bot<TOKEN>/sendMessage" \
  -d "chat_id=<CHAT_ID>&text=/status"
```

---

## 🚀 Production Deployment

### Systemd Service

Создай `/etc/systemd/system/telegram-bot.service`:

```ini
[Unit]
Description=Lead Generation Telegram Bot
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/lead-generation-system
Environment="PATH=/opt/lead-generation-system/venv/bin"
ExecStart=/opt/lead-generation-system/venv/bin/python -m shared.telegram_bot
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
```

**Запуск:**
```bash
sudo systemctl enable telegram-bot
sudo systemctl start telegram-bot
sudo systemctl status telegram-bot
```

---

## 📚 Файлы

- `shared/telegram_bot.py` - основной модуль бота
- `shared/telegram_notifier.py` - модуль уведомлений
- `scripts/test_telegram_bot.py` - тестовый скрипт
- `scripts/start_telegram_bot.sh` - скрипт запуска

---

## ✅ Checklist

- [ ] Бот создан через @BotFather
- [ ] TOKEN и CHAT_ID в .env
- [ ] Бот запущен: `python -m shared.telegram_bot`
- [ ] Тесты пройдены: `python scripts/test_telegram_bot.py`
- [ ] Команды работают в Telegram
- [ ] Уведомления приходят при ошибках
- [ ] Health check работает

---

**Готово! Теперь у тебя полноценный интерактивный бот для мониторинга системы! 🚀**

