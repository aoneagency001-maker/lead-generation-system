# 🏗️ Lead Generation System - Architecture
...
```
┌─────────────────────────────────────────────────────────────┐
│                    Command Center (crewAI)                   │
│              Orchestrates all modules and agents             │
└─────────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        │                   │                   │
...
│    Market     │   │    Traffic    │   │     Lead      │
│   Research    │──▶│  Generation   │──▶│ Qualification │
└───────────────┘   └───────────────┘   └───────────────┘
        │                   │                   │
        │                   │                   ▼
        │                   │           ┌───────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│                   Supabase (PostgreSQL)                      │
│         niches │ campaigns │ ads │ leads │ conversations    │
└─────────────────────────────────────────────────────────────┘
```
...
Calculate Qualification Score
    ↓
If Qualified → Module 4 (Handoff)
If Disqualified → Mark and Archive
```
...
- Profitability Score = (Avg Price - Avg Cost) × Volume
...
## 🔄 Data Flow Example: Complete Cycle
...
User: "Create OLX campaign, budget 50,000 KZT"
    ↓
Generate ad copy (LLM): "Быстрый ремонт ноутбуков..."
    ↓
...
```
Lead responds on WhatsApp: "Сколько стоит ремонт?"
    ↓
Bot: "Добрый день! Зависит от поломки. Что случилось?"
Lead: "Не включается, залил водой"
    ↓
Bot: "Понятно. Когда нужен ремонт?"
Lead: "Срочно, завтра экзамен"
    ↓
Bot: "Бюджет примерно 10-15 тысяч. Подходит?"
Lead: "Да, нормально"
    ↓
...
DigitalOcean Droplet (8GB RAM, 4 vCPU):
  ...
  ├── Docker Compose (Supabase, Redis, n8n, Metabase)
  ├── WAHA (WhatsApp API)
...
### Why FastAPI?
- ✅ Async/await support (critical for I/O-heavy workload)
...
- ✅ Free tier (5GB storage, 500MB database)

### Why crewAI?
- ✅ Multi-agent orchestration (perfect for modular system)
- ✅ LangChain integration (LLM flexibility)
- ✅ Task delegation (agents work together)
- ✅ Python-native (same stack)

### Why Playwright over Selenium?
- ✅ Modern API (async/await)
- ✅ Better anti-detect capabilities
- ✅ Faster execution
